...
export default function EmailForm() {
  ...
  const addPerson = () => {
    setFormData({
      ...formData,
      people: [...formData.people, { id: crypto.randomUUID(), name: "", email: "" }],
    })
  }
  ...
  const updatePerson = (id: string, field: "name" | "email", value: string) => {
    setFormData({
      ...formData,
      people: formData.people.map((person) => (person.id === id ? { ...person, [field]: value } : person)),
    })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://swapnil11.app.n8n.cloud/webhook-test/cold-email-hook", {
        method: "POST",
        headers: {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Textarea
            ...
            className={`min-h-32 ${errors.jobDescription ? "border-destructive" : ""}`}
          />
          {errors.jobDescription && <p className="text-destructive text-sm mt-1">{errors.jobDescription}</p>}
        </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div>
                        ...
                        <Input
                          ...
                          className={
                            errors.people && errors.people[person.id] && errors.people[person.id].name
                              ? "border-destructive"
                              : ""
                          }
                        />
                        {errors.people && errors.people[person.id] && errors.people[person.id].name && (
                          <p className="text-destructive text-sm mt-1">{errors.people[person.id].name}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          className={
                            errors.people && errors.people[person.id] && errors.people[person.id].email
                              ? "border-destructive"
                              : ""
                          }
                        />
                        {errors.people && errors.people[person.id] && errors.people[person.id].email && (
                          <p className="text-destructive text-sm mt-1">{errors.people[person.id].email}</p>
                        )}
                      </div>
                    </div>
                  </div>
